...
from arkui_ut_agent.environments.docker import DockerEnvironment
from arkui_ut_agent.exceptions import Submitted
from arkui_ut_agent.run.benchmarks.programbench import copy_submission, main

# Lightweight image used for the real-docker tests. Already cached on machines
# that run arkui-ut-code-agent's docker test suite (see tests/environments/test_docker.py).
...
def test_programbench_end_to_end_real_docker(real_programbench_first_instance, tmp_path, container_executable):
    ...
    with patch("arkui_ut_agent.run.benchmarks.programbench.get_model", side_effect=lambda **kw: _SubmittingModel()):
        main(
            slice_spec="",
...
def test_programbench_skip_existing_real_docker(real_programbench_first_instance, tmp_path, container_executable):
    ...
    with (
        patch("arkui_ut_agent.run.benchmarks.programbench.get_model") as mock_get_model,
        patch("arkui_ut_agent.run.benchmarks.programbench.get_environment") as mock_get_env,
    ):
        main(
            slice_spec="",
            filter_spec=f"^{iid}$",
