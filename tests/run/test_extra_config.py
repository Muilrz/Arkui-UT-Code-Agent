...
from arkui_ut_agent.run.utilities.config import app, configure_if_first_time, edit, set, setup, unset


@pytest.fixture(autouse=True)
def no_reload(monkeypatch):
    monkeypatch.setattr("arkui_ut_agent.run.utilities.config._reload_config", lambda: None)
...
class TestConfigSetup:
    ...
    def test_setup_with_all_inputs(self, tmp_path):
        ...
        with (
            patch("arkui_ut_agent.run.utilities.config.global_config_file", config_file),
            patch("arkui_ut_agent.run.utilities.config.prompt") as mock_prompt,
            patch("arkui_ut_agent.run.utilities.config.console.print"),
        ):
    ...
    def test_setup_key_name_but_no_value(self, tmp_path):
        ...
        with (
            patch("arkui_ut_agent.run.utilities.config.global_config_file", config_file),
            patch("arkui_ut_agent.run.utilities.config.prompt") as mock_prompt,
            patch("arkui_ut_agent.run.utilities.config.console.print") as mock_print,
        ):
...
class TestConfigSet:
    ...
    def test_set_with_both_arguments_provided(self, tmp_path):
        ...
        with patch("arkui_ut_agent.run.utilities.config.global_config_file", config_file):
...
class TestConfigureIfFirstTime:
    ...
    def test_configure_when_not_configured(self, tmp_path):
        ...
        with (
            patch("arkui_ut_agent.run.utilities.config.global_config_file", config_file),
            patch("arkui_ut_agent.run.utilities.config.setup") as mock_setup,
            patch("arkui_ut_agent.run.utilities.config.console.print") as mock_print,
