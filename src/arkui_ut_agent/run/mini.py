...
"""Run arkui-ut-code-agent in your local environment. The executable is `arkui-ut-agent`."""
# Read this first: https://github.com/Muilrz/Arkui-UT-Code-Agent  (usage)
...
from arkui_ut_agent.agents import get_agent
from arkui_ut_agent.agents.utils.prompt_user import _multiline_prompt
from arkui_ut_agent.config import builtin_config_dir, get_config_from_spec
from arkui_ut_agent.environments import get_environment
from arkui_ut_agent.models import get_model
from arkui_ut_agent.run.utilities.config import configure_if_first_time
from arkui_ut_agent.utils.serialize import UNSET, recursive_merge
...
_HELP_TEXT = """Run arkui-ut-code-agent in your local environment.

[not dim]
More information about the usage: [bold green]https://github.com/Muilrz/Arkui-UT-Code-Agent[/bold green]
[/not dim]
"""
...
def main(
    ...
    model_class: str | None = typer.Option(None, "--model-class", help="Model class to use (e.g., 'litellm' or 'arkui_ut_agent.models.litellm_model.LitellmModel')", rich_help_panel="Advanced"),
    agent_class: str | None = typer.Option(None, "--agent-class", help="Agent class to use (e.g., 'interactive' or 'arkui_ut_agent.agents.interactive.InteractiveAgent')", rich_help_panel="Advanced"),
    environment_class: str | None = typer.Option(None, "--environment-class", help="Environment class to use (e.g., 'local' or 'arkui_ut_agent.environments.local.LocalEnvironment')", rich_help_panel="Advanced"),
